...
@Component({
  selector: 'app-owners-relationshiprevenuesgeneral',
  templateUrl: './owners-relationshiprevenuesgeneral.component.html',
  styleUrls: ['./owners-relationshiprevenuesgeneral.component.css'],
})
export class OwnersRelationshiprevenuesgeneralComponent implements OnInit {
  ...
  openExternalLink(): void {
    let endpoint = 'general-relationshiprevenues';
    const company = this.getCompany();
    if (endpoint) {
      const data = {
        codigo_empresa: company,
        usuario: this.user,
        primeraFecha: this.infoForm.value.firstDate,
        ultimaFecha: this.infoForm.value.lastDate,
        empresas: this.selectedOwners,
      };
